...
main() = {
    ...
    # fromJsonValue supports parameterized types in fields:
    # List[T], Option[T], Map[K,V], and nested combinations
    # Type params in generic types are substituted automatically
    # e.g., Wrapper[T] = { items: List[T] } with "Wrapper[Int]"
}
```
